...
trainData <- subset(trainData, select=-c(id,ultima_modificacion,seccion,area_seccion,lat,long))

trainData

rf <- randomForest(inclinacion_peligrosa ~ .,data=trainData, importance=TRUE)
pred <- predict(rf, newData=trainData)

print(rf)
importance(rf)


trainData$prediction <- pred
